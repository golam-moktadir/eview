                        ...
                        <div class="box-body">
                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-sm-1 col-xs-4" style="padding-left: 0px; margin-left: 0px; padding-top: 26px; ">
                                    <a href="<?php echo base_url(); ?>Show_form/patient/amb" title="Add Patient"
                                       class="form-control btn btn-info waves-effect waves-light"><i class="fa fa-plus"></i>
                                    </a>
                                </div>
                                <div class="form-group col-sm-2 col-xs-12">
                                    <label for="duration">Total Stay Duration</label>
                                    <input type="text" name="duration" id="duration" class="form-control">
                                </div>
                                <div class="form-group col-sm-2 col-xs-12">
                                    <label for="receivable">Total Receivable</label>
                                    <input type="text" name="receivable" id="receivable" class="form-control">
                                </div>
                                <div class="form-group col-sm-2 col-xs-12">
                                    <label for="payable">Total Payable Charge</label>
...
<script>
    view();
    $("#cabin_form").on("submit", function (e) {
        e.preventDefault();
        if (confirm("Are you sure?")) {
            var url = "<?php echo base_url() ?>Insert_data/cabin";
            $.ajax({
                url: url,
                type: "post",
                dataType: "json",
    ...
    function edit_data(id) {
        var url = "<?php echo base_url() ?>Get_data/cabin";
        $.ajax({
            url: url,
            type: "post",
            dataType: "json",
            ...
            success: function (data) {
                ...
                $('#receivable').val(data[0].receivable);
    ...
    $("#click_edit").on("click", function () {
        if (confirm("Are you sure?")) {
            var url = "<?php echo base_url() ?>Edit_data/cabin";
            $.ajax({
                url: url,
                type: "post",
                dataType: "json",
                data: new FormData($('#cabin_form')[0]),
                processData: false,
                contentType: false,
                success: function (data) {
                    $("#cabin_form")[0].reset();
    ...
    function delete_data(id) {
        if (confirm("Are you sure?")) {
            var url = "<?php echo base_url() ?>Delete_data/cabin";
            $.ajax({
                url: url,
                type: "post",
                dataType: "json",
    ...
    function view() {
        $.ajax({
            url: "<?php echo base_url() ?>View_data/cabin",
            dataType: "json",
            success: function (data) {
